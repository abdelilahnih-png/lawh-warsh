const CACHE_NAME = "lawh-warsh-shell-v3";
const RUNTIME_CACHE = "lawh-warsh-runtime-v1";
const SHELL_FILES = [
  "./manifest.json",
  "./icons/icon-192.png",
  "./icons/icon-512.png"
...
self.addEventListener("fetch", (event) => {
  ...
  // ---- مصادر خارجية (صوت / تفسير / مواقيت الصلاة) ----
  if (!isSameOrigin) {
    if (isAudioRequest(url) || isTafsirRequest(url)) {
      // Cache-first: المحتوى ثابت (نفس الآية = نفس التلاوة/نفس التفسير ديما)،
      // أول مرة كتنسمع/كتنقرا، من بعد كتخدم أوفلاين مباشرة
      event.respondWith(
    ...
    return; // باقي المصادر الخارجية (Supabase، الخطوط...) — بلا تدخل، بحال قبل
  }
