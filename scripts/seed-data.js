 ...
 *  ملاحظة مهمة: quran-meta ماعندهاش دالة Thumun al-Hizb أصلية لـ Warsh — هاد التقسيم (480)
 *  موجود غير فموديول Qalun (موثق وموثوق، مبني على حدود حقيقية، ماشي تنصيف رياضي).
 *  Qalun وWarsh كيشاركو بالضبط نفس ترقيم الآيات (رواية نافع، 6214 آية)، فحدود
 *  الثمن ديال Qalun صحيحة 100% مع نص Warsh. كنا قبل كنبنيو الحدود يدويا بتنصيف
 *  كل ربع حزب بعدد الآيات (buildThumunBoundaries) — هاد الطريقة كانت السبب فبوق
 *  التقسيم فكل صفحات المصحف، حيت التنصيف الرياضي ماشي هو التقسيم التقليدي الحقيقي. */
async function computeAccurateDivisions(ayahs) {
  const { getAyahMeta, findAyahIdBySurah, meta } = await import("quran-meta/warsh");
  const { findThumunAlHizb, meta: qalunMeta } = await import("quran-meta/qalun");
  console.log(`📚 quran-meta (ورش): ${meta.numAyahs} آية، ${meta.numRubAlHizbs} ربع حزب`);
  console.log(`📚 quran-meta (Qalun — لحدود الثمن الحقيقية): ${qalunMeta.numThumunAlHizbs} ثمن`);
  ...
  for (const a of ayahs) {
    ...
    let thumn = null;
    try {
      thumn = findThumunAlHizb(a.surah_number, a.ayah_number);
    } catch (e) {
      thumn = null;
    }
    a.thumn_number = thumn;
    a.page_number = a.thumn_number;
    if (thumn == null) mismatches++;
  }
